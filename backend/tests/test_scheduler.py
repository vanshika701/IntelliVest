"""Unit test — verifies the scheduler registers the expected jobs and can
be started/stopped cleanly, without waiting for any job to actually fire
(that would mean waiting up to an hour in a test)."""

from app.core.scheduler import scheduler, start_scheduler, stop_scheduler


async def test_start_scheduler_registers_all_expected_jobs() -> None:
